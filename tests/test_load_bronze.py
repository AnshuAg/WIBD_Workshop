...
Bronze's only contract is "load the raw file exactly as it is" — no casting,
filtering, deduping, or other judgment calls. These tests check that contract
against a small synthetic fixture workbook, not the real 541,909-row dataset —
the logic under test should hold for any workbook shaped like the source, not
just today's data.
"""
...
def test_loads_every_row_from_the_source_workbook(tmp_path):
    ...
    # every row in the source workbook made it into bronze — no filtering.
...
def test_does_not_coerce_or_reformat_values(tmp_path):
    ...
    # (numeric round-trip loses precision) — both should survive as literal text.
    xlsx_path = tmp_path / "fixture.xlsx"
    _write_fixture_workbook(
        xlsx_path,
...
def test_does_not_drop_rows_with_missing_values(tmp_path):
    # a guest-checkout-shaped row with no CustomerID.
    xlsx_path = tmp_path / "fixture.xlsx"
    _write_fixture_workbook(
        xlsx_path,
    ...
    # the row is kept, with the missing value preserved as NULL.
    assert row_count == 1
    assert null_count == 1


def test_does_not_dedupe_identical_rows(tmp_path):
    # the same row twice, as a re-scan/re-export would produce.
    xlsx_path = tmp_path / "fixture.xlsx"
    ...
    # both copies are kept — dedup is Silver's decision, not Bronze's.
    assert row_count == 2
